<?php
/**
 * Request Form API Endpoint
 * 
 * This endpoint provides detailed information about a specific request,
 * including its type, requirements, and form fields needed for filling out forms.
 * GET: View request details with type-specific information for form filling
 */
...
error_log("Starting request form endpoint. Response ID: " . $responseId);
...
if ($method === 'GET') {
    // Get request details with type information for form filling
    try {
        error_log("Getting request form details for tracking number: " . $tracking_number . " and user ID: " . $user_id);
        $result = $request->getRequestDetailsWithType($tracking_number, $user_id, true);
        ...
        error_log("Error getting request form details: " . $e->getMessage());
        http_response_code(500);
        echo json_encode([
            'status' => 'error',
            'message' => 'Failed to fetch request form details: ' . $e->getMessage(),
            'code' => 500
        ]);
    }
} else {
