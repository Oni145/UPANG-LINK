...
const requestTypeNames = {
  ...
  function viewRequest(requestId) {
    ...
    if (request.note && request.note.trim() !== "") {
      ...
      displayCommentTextEl.textContent = "No comment was available";
    }
    
    // Hide all editing elements since commenting is disabled
    editCommentTextarea.style.display = 'none';
    saveTicketCommentBtn.style.display = 'none';
    updateTicketCommentBtn.style.display = 'none';
    cancelEditCommentBtn.style.display = 'none';
    
    // Ensure the comment is always displayed but not editable
    displayCommentTextEl.style.display = 'block';
    
    // Prevent the pointer from looking clickable
    displayCommentTextEl.style.cursor = 'default';
    
    /* 
    // REMOVE OR COMMENT OUT THE EVENT LISTENER TO DISABLE EDITING
    displayCommentTextEl.addEventListener('click', () => {
        editCommentTextarea.value = (displayCommentTextEl.textContent.trim() === "No comment was available") ? "" : displayCommentTextEl.textContent;
